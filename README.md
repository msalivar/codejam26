...
You also can't win too early. Skipping is only possible after at least 30% of the ad has played (capped at 30 seconds on longer ads), and only after typing enough characters for the ad's length (roughly 3 characters per second, floored at 20 and capped at 120 — so a 30s ad requires ~100 characters). The character requirement is waived once 60% of the ad has played or 60 seconds have elapsed, whichever comes first. Before those gates open, the skip bar is hard-capped just below the ad bar no matter how fast you type.
